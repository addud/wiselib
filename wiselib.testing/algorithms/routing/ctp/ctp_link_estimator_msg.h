...
namespace wiselib {
	...
	*/
	...
	public:
		...
		enum HeaderSize {
			HEADER_SIZE = sizeof(message_id_t)+sizeof(uint8_t) + sizeof(uint8_t)
		};

		// --------------------------------------------------------------------

		inline CtpLinkEstimatorMsg(message_id_t id) {
			set_msg_id(id);
		}
	...
	private:
		...
		enum data_positions {
			ID_POS=0,
			NE_POS = ID_POS+sizeof(message_id_t), 
			SEQNO_POS = NE_POS + sizeof(uint8_t), 
			FOOTER_POS = SEQNO_POS + sizeof(uint8_t)
		};
