...
template<typename OsModel_P, typename LinkEstimator_P, typename RoutingTable_P,
...
public:
	...
	typedef CtpRoutingEngine<OsModel, LinkEstimator, RoutingTable, RandomNumber,
	...
	int enable(void) {
		...
		le_->template reg_event_callback<self_type, &self_type::rcv_event>(
				this);
		...
		if (minEtx != MAX_METRIC) {
			if (currentEtx == MAX_METRIC
				...
				if (best_found) {
					...
					echo("%d: EROR. Trying to use a NULL best routing node.",
							self);
				}
			}
		}
...
// -----------------------------------------------------------------------------------
	...
	error_t routingTableUpdateEntry(node_id_t from, node_id_t parent,
		...
		RoutingTableIterator it;
		ctp_etx_t linkEtx;
		linkEtx = evaluateEtx(le_->command_LinkEstimator_getLinkQuality(from));
...
// ----------------------------------------------------------------------------------
	...
	void printRoutingTable() {
		...
		echo("Routing table:");
		for (it = routingTable.begin(); it != routingTable.end(); it++) {
			echo("neighbour = %d, parent = %d, etx = %d", it->first,
					it->second.parent, it->second.etx);
		}
